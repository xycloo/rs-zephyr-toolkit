...
use crate::{
    env::EnvClient,
    external::{
        read_account_from_ledger, read_contract_data_entry_by_contract_id_and_key,
...
impl EnvClient {
    fn express_and_deser_entry<'a, D: Deserialize<'a> + Into<O>, O>(
        ...
        let deser = bincode::deserialize::<Option<D>>(slice).map_err(|_| SdkError::Conversion)?;
        if deser.is_none() {
            return Ok(None);
        }
    ...
    pub fn read_contract_instance(
        ...
        Self::express_and_deser_entry::<ContractDataEntryStellarXDR, ContractDataEntry>(
            status, offset, size,
        )
    }
    ...
    >(
        ...
        let resp = Self::express_and_deser_entry::<ContractDataEntryStellarXDR, ContractDataEntry>(
            status,
            inbound_offset,
            inbound_size,
        )?;
